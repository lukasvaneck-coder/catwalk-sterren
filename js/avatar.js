...
const Avatar = (() => {
  ...
  function makeCtx(look, opts = {}) {
    ...
    const ctx = { id, defs, skin: skin.c, skinShade: skin.shade, eye: eye.c, expr: opts.expr || 'smile' };
  ...
  function headOnly(ctx) {
    ...
    return `<circle cx="97" cy="108" r="10" fill="${s}"/><circle cx="203" cy="108" r="10" fill="${s}"/>
      ...
      <ellipse cx="150" cy="100" rx="54" ry="60" fill="${s}"/>`;
  }
  ...
  const HAIR = {
    ...
    staartjes: {
      back: c => `<path d="M96 110 Q66 150 76 206 M204 110 Q234 150 224 206" stroke="${c.hairFill}" stroke-width="24" stroke-linecap="round" fill="none"/>`,
      front: c => `<path d="${CAP} Q150 64 92 104 Z" fill="${c.hairFill}"/><circle cx="96" cy="112" r="7" fill="#ff5da2"/><circle cx="204" cy="112" r="7" fill="#ff5da2"/>`,
    },
    krullen: {
  ...
  const HAND_BOX = { teddy: '46 280 84 84', icecream: '56 262 70 90', beachball: '40 290 66 66', football: '46 294 58 58', umbrella: '0 136 116 190', guitar: '60 150 190 200', wand: '196 196 84 114', pumpkin: '58 282 58 70', telescope: '196 196 96 116', gift: '56 296 56 56', balloons: '206 186 80 130', sword: '188 216 70 108', mic: '198 234 50 76', bouquet: '60 246 60 62' };
  ...
  function thumbBox(item) {
    ...
    if (item.cat === 'hair' && !HAIR[item.shape].back) return '60 0 180 200';
  ...
  function render(look, outfit, opts = {}) {
    ...
    if (opts.bg !== false) { const b = get('bg'); L.push(b && Backgrounds[b.shape] ? Backgrounds[b.shape](ctx) : `<rect width="300" height="520" fill="${opts.plainBg || 'transparent'}"/>`); }
    L.push(partOf(ctx, get('back'), 'back'), partOf(ctx, get('bag'), 'back'), partOf(ctx, get('hat'), 'back'));
    L.push(H.back ? H.back(ctx) : '');
    L.push(body(ctx));
    // niets aan? dan een simpel wit hemdje en short
    if (!dress && !get('bottom')) L.push(BOTTOMS.shorts(ctx, { c: ['#fdfdfd', '#e6e6ee'] }));
    if (!dress) L.push(partOf(ctx, get('bottom'), 'front'));
    L.push(partOf(ctx, get('shoes'), 'front'));
    if (!dress && !get('top')) L.push(TOPS.tank(ctx, { c: ['#fdfdfd'] }));
    L.push(dress ? partOf(ctx, dress, 'front') : partOf(ctx, get('top'), 'front'));
    L.push(partOf(ctx, get('back'), 'front'), partOf(ctx, get('bag'), 'front'), partOf(ctx, get('neck'), 'front'));
    L.push(face(ctx, outfit));
    L.push(H.front(ctx));
    L.push(partOf(ctx, get('hat'), 'front'), partOf(ctx, get('glasses'), 'front'), partOf(ctx, get('hand'), 'front'), partOf(ctx, get('pet'), 'front'));
  ...
  return { render, thumb, light, dark, starPath, heartPath, flower };
})();
